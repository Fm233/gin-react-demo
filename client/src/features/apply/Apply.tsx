...
const Apply: React.FC = () => {
  ...
  const alert = inError ? (
    <Alert style={{ margin: "5px 0" }} message={errorMessage} type="error" />
  ) : undefined;
  return (
    <div style={{ margin: "300px auto", maxWidth: "400px" }}>
      <Search
        ...
        onSearch={async (raw: string) => {
          const reg: RegExp = /https:\/\/www\.bilibili\.com\/video\/(.{12})/g;
          const regRes = reg.exec(raw);
          const value = regRes === null || regRes.length <= 1 ? raw : regRes[1];
          const bv = value.startsWith("BV")
            ? value
            : value.startsWith("bv")
            ? "BV" + value.substring(2)
            : "BV" + value;
          if (bv.length !== 12) {
            setInError(true);
